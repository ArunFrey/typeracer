...
def plot_hist(username, min_races = 100):
    ...
    fig.add_vline(x = np.mean(df['wpm'][0:20]), 
                  line_dash = "dot",
...
def main(): 
    ...
    parser = argparse.ArgumentParser(
        description='Plotting data for typeracer user.')
    parser.add_argument('-u', '--user_name', type=str,
                        required=True, help='Enter the username of the user whose data you want to plot.')
    args = parser.parse_args()

    fig = make_subplots(rows=1, cols=2)
    
    figures = [
        plot_wpm_over_time(args.user_name),
        plot_hist(args.user_name)
    ]
